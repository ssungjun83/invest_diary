...
def get_conn() -> sqlite3.Connection:
    ...
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS value_chain_store (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
...
def load_snapshot_exact(snapshot_date: date) -> pd.DataFrame:
    conn = get_conn()
    try:
        df = _query_snapshot_for_date(conn, snapshot_date)
    finally:
...
def load_snapshot_cash_exact(snapshot_date: date) -> tuple[float, float]:
    conn = get_conn()
    try:
        row = conn.execute(
            """
...
def fetch_price_snapshot_from_yahoo_quote(ticker: str) -> tuple[float | None, str, str]:
    tkr = clean_valid_ticker(ticker)
    if not tkr:
        return None, "", "티커 없음"
    try:
        ...
        return None, "", f"yahoo.quote 조회 실패: {exc}"

    results = ((payload.get("quoteResponse") or {}).get("result") or [])
    if not results:
        return None, "", "yahoo.quote 결과 없음"
    item = results[0] if isinstance(results[0], dict) else {}
    if not isinstance(item, dict):
        return None, "", "yahoo.quote 형식 오류"

    price = None
    for key in ["regularMarketPrice", "postMarketPrice", "preMarketPrice", "previousClose"]:
        val = _safe_to_float(item.get(key))
        if val is not None and val > 0:
            price = float(val)
            break
    currency = str(item.get("currency") or "").strip().upper()
    if price is None:
        return None, currency, "yahoo.quote 주가 없음"
    return price, currency, "yahoo.quote"
...
def fetch_current_price_krw_from_ticker(ticker: str, rate_date: date | None = None) -> tuple[float | None, str]:
    ...
    try:
        ...
        yf = None
        errors.append(f"yfinance 미설치/실패: {exc}")

    obj = yf.Ticker(tkr) if yf is not None else None
    if obj is not None:
        try:
            fast = obj.fast_info or {}
        except Exception as exc:
            fast = {}
            errors.append(f"fast_info 실패: {exc}")
        ...
        if price_native is None:
            ...
            for _ in range(2):
                try:
                    ...
                    errors.append(f"history 실패: {exc}")
                    hist = pd.DataFrame()
        ...
        if not currency:
            try:
                ...
                info = {}
                errors.append(f"info 실패: {exc}")
            currency = str(info.get("currency") or "").strip().upper()

    if price_native is None or price_native <= 0:
        quote_price, quote_currency, quote_src = fetch_price_snapshot_from_yahoo_quote(tkr)
        if quote_price is not None and quote_price > 0:
            price_native = float(quote_price)
            source_label = quote_src
            if not currency and quote_currency:
                currency = quote_currency
        else:
            errors.append(quote_src or "yahoo.quote 실패")
    ...
    if price_native is None or price_native <= 0:
        err_text = " / ".join([str(msg).strip() for msg in errors if str(msg).strip()])
        return None, err_text or "현재 주가를 찾지 못했습니다."
...
def _build_value_chain_sankey_figure(match_rows: list[dict], chain_name: str):
    ...
    for n in nodes:
        ...
        elif " | " in n and any(s in n for s in stage_order):
            parts = [p.strip() for p in n.split(" | ")]
            stage_name = next((p for p in parts if p in stage_color), "기타")
...
def backfill_company_list_price_by_ticker(
    ticker: str,
    price_krw: float,
    price_source: str = "ticker_backfill",
) -> int:
    tkr = clean_valid_ticker(ticker)
    price_val = _safe_to_float(price_krw)
    if not tkr or price_val is None or price_val <= 0:
        return 0

    now_str = datetime.now().isoformat(timespec="seconds")
    src = str(price_source or "ticker_backfill").strip() or "ticker_backfill"
    ...
    try:
        ...
        for row in rows:
            old_price = _safe_to_float(row[3])
            ...
            _archive_company_list_state(
                ...
                price_updated_at=str(row[5] or ""),
                source=str(row[6] or ""),
                action="ticker_price_backfill_before",
            )
        ...
        result = conn.execute(
            ...
            SET price_krw = ?,
                ...
                updated_at = ?
            WHERE ticker = ?
              AND (price_krw IS NULL OR price_krw <= 0)
            """,
            (float(price_val), src, now_str, now_str, tkr),
        )
        conn.commit()
    ...
    *,
    ...
    if normalized_provider == "claude":
        ...
        for candidate_model in model_candidates:
            ...
            if not err_msg:
                return _extract_claude_text_from_response(data), ""
            errors.append(f"{candidate_model}: {err_msg}")
...
def _get_runtime_api_settings() -> dict[str, str]:
    ...
    openai_key_source = "session/db:global_openai_api_key" if openai_key_session else (
        openai_key_direct_source or "session/db:global_openai_api_key"
    )
    ...
    claude_key_source = "session/db:global_claude_api_key" if claude_key_session else (
        claude_key_direct_source or "session/db:global_claude_api_key"
    )
    ...
    alpha_key_source = "session/db:global_alpha_vantage_api_key" if alpha_key_session else (
        alpha_key_direct_source or "session/db:global_alpha_vantage_api_key"
    )
    ...
    finnhub_key_source = "session/db:global_finnhub_api_key" if finnhub_key_session else (
        finnhub_key_direct_source or "session/db:global_finnhub_api_key"
    )
...
def render_input_tab(selected_date: date, edited_df: pd.DataFrame, usd_krw_rate: float) -> pd.DataFrame:
    ...
    with cash_col3:
        if st.button("예수금 저장", key=f"save_cash_only_btn_{cash_key_suffix}"):
            ...
            # 보유종목 스냅샷이 없는 날짜에서 예수금만 저장할 때는 원격 보유현황이 비어 덮어써지는 것을 막기 위해
            # GitHub 즉시 동기화를 생략하고, 스냅샷 저장 시점에 함께 반영한다.
            if has_snapshot_on_date(selected_date):
                sync_target_df = ensure_portfolio_columns(load_snapshot_exact(selected_date), usd_krw_rate, force_usd_rate=True)
    ...
    view_krw = to_krw_view(cleaned_df, usd_krw_rate)
    stock_total_value = float(pd.to_numeric(view_krw[COL_VALUE_KRW], errors="coerce").fillna(0).sum()) if not view_krw.empty else 0.0
    total_pnl = float(pd.to_numeric(view_krw[COL_PNL_KRW], errors="coerce").fillna(0).sum()) if not view_krw.empty else 0.0
    cash_krw = float(input_cash_krw or 0.0)
    cash_usd = float(input_cash_usd or 0.0)
    cash_total_krw = cash_krw + cash_usd * float(usd_krw_rate)
    total_value = stock_total_value + cash_total_krw
    total_principal = total_value - total_pnl
    total_return = (total_pnl / total_principal * 100.0) if total_principal else 0.0
...
def render_company_analysis_tab(current_df: pd.DataFrame) -> None:
    ...
    if not company_list_df.empty:
        for _, row in company_list_df.iterrows():
            ...
            if not nm:
                continue
            tkr = clean_valid_ticker(str(row.get("ticker") or ""))
            ticker_map[nm] = tkr
            ...
            if price_val is not None and price_val > 0:
                ...
                if tkr:
                    ticker_price_map[tkr] = float(price_val)
    ...
    for nm in sorted(holding_set | listed_set):
        ...
        if nm in listed_set:
            tags.append("추가리스트")
        tkr = ticker_map.get(nm, "")
        price_view = price_map.get(nm)
        if (price_view is None or float(price_view) <= 0) and tkr:
            price_view = ticker_price_map.get(tkr)
        overview_rows.append(
            {
                "기업명": nm,
                "티커": tkr,
                "산업섹터": sector_map.get(nm, ""),
                "현재주가(원화환산)": price_view,
        ...
        if not targets:
            ...
            with st.spinner("보유종목 현재 주가를 불러오는 중입니다..."):
                for row in targets:
                    ...
                    ticker_backfilled_count += backfill_company_list_price_by_ticker(
                        ticker=ticker,
                        price_krw=float(price_krw),
                        price_source=price_src or "api",
                    )
            ...
            if updated_count > 0:
                backfill_note = f", 티커공유 보정 {ticker_backfilled_count}개" if ticker_backfilled_count > 0 else ""
                st.session_state["analysis_bulk_price_notice"] = (
                    f"현재 주가 업데이트 완료: {updated_count}개{backfill_note} (기준: {price_target_source}){auto_save_suffix}"
                )
            else:
                st.session_state["analysis_bulk_price_notice"] = (
...
def save_value_chain_to_db(chain_name: str, chain_data: dict) -> int:
    now = datetime.now().isoformat(timespec="seconds")
    conn = get_conn()
    try:
        cur = conn.execute(
            "INSERT INTO value_chain_store (chain_name, chain_data_json, created_at, updated_at) VALUES (?, ?, ?, ?)",
            (chain_name, json.dumps(chain_data, ensure_ascii=False), now, now),
        )
        conn.commit()
...
def update_value_chain_in_db(chain_id: int, chain_name: str, chain_data: dict) -> None:
    now = datetime.now().isoformat(timespec="seconds")
    conn = get_conn()
    try:
        conn.execute(
            "UPDATE value_chain_store SET chain_name=?, chain_data_json=?, updated_at=? WHERE id=?",
            (chain_name, json.dumps(chain_data, ensure_ascii=False), now, chain_id),
        )
        conn.commit()
    finally:
        conn.close()
...
def load_value_chain_list_from_db() -> list:
    conn = get_conn()
    try:
        rows = conn.execute(
            "SELECT id, chain_name, created_at, updated_at FROM value_chain_store ORDER BY updated_at DESC"
        ).fetchall()
        return [{"id": r[0], "chain_name": r[1], "created_at": r[2], "updated_at": r[3]} for r in rows]
    finally:
        conn.close()
...
def load_value_chain_by_id_from_db(chain_id: int) -> dict:
    ...
    try:
        row = conn.execute(
            "SELECT chain_data_json FROM value_chain_store WHERE id=?", (chain_id,)
        ).fetchone()
...
def delete_value_chain_from_db(chain_id: int) -> None:
    conn = get_conn()
    try:
        conn.execute("DELETE FROM value_chain_store WHERE id=?", (chain_id,))
        conn.commit()
    finally:
        conn.close()
...
def build_value_chain_keyword_index(chain_data: dict) -> dict:
    ...
    반환:
      {
        "업스트림":   {"type":"stage",   "companies":[...], "segments":[...]},
        "운반":       {"type":"segment", "stage":"미드스트림", "companies":[...]},
        "삼성중공업": {"type":"company", "ticker":..., "matched":bool,
                       "matched_company":..., "sector":..., "entries":[{"stage":..,"segment":..}]},
      }
    """
    idx: dict = {}
    all_stages: list[str] = []
    ...
    for mr in (chain_data.get("matched_rows") or []):
        nm = str(mr.get("input_company") or "").strip()
        if nm:
            match_map[nm] = mr
    ...
    for row in rows:
        ...
        for c in companies:
            ...
            if not isinstance(comp_entry, dict) or str(comp_entry.get("type") or "") != "company":
                ...
                idx[c] = {
                    "type": "company",
                    "ticker": str(mr_info.get("ticker") or "").strip(),
                    "sector": str(mr_info.get("sector") or "").strip(),
                    "matched": bool(mr_info.get("matched")),
...
def render_value_chain_tab() -> None:
    ...
    st.caption(
        ...
        "관심기업(기업정보 리스트)과 자동 매칭해서 저장·탐색합니다."
    )

    # ── 세션 상태 초기화 ─────────────────────────────────────────────────────
    for _k, _v in [
        ("value_chain_uploader_nonce", 0),
        ("value_chain_pending_result", {}),
        ("value_chain_notice", ""),
        ("value_chain_warning", ""),
    ...
    # ── 알림 표시 ────────────────────────────────────────────────────────────
    for _k, _fn in [
        ("value_chain_notice", st.success),
        ("value_chain_warning", st.warning),
        ("value_chain_summary_notice", st.success),
        ("value_chain_summary_warning", st.warning),
    ]:
    ...
    with st.expander("새 이미지로 밸류체인 분석", expanded=(sel_value == NEW_KEY)):
        ...
        with up_col2:
            vc_file = st.file_uploader(
                "밸류체인 이미지 (PNG/JPG/WEBP)",
                type=["png", "jpg", "jpeg", "webp"],
                key=uploader_key,
            )
        with btn_col2:
            st.write("")
            run_btn = st.button("이미지 분석", type="primary", key="value_chain_run_btn")
        if vc_file is not None:
            st.image(vc_file, caption="업로드된 이미지", use_container_width=True)
        if run_btn:
            if vc_file is None:
                ...
                with st.spinner("밸류체인 이미지에서 단계/기업을 추출하고 매칭 중입니다..."):
                    ...
                    if parse_err:
                        ...
                        company_list_df_tmp = load_company_list()
                        match_rows_tmp = _build_value_chain_match_rows(parsed, company_list_df_tmp)
                        parsed["matched_rows"] = match_rows_tmp
                        ...
                        matched_cnt = sum(1 for r in match_rows_tmp if bool(r.get("matched")))
                        st.session_state["value_chain_notice"] = (
                            f"분석 완료: 기업 {total_nodes}개 중 관심기업 매칭 {matched_cnt}개 — 아래에서 저장하세요."
                        )
                        st.rerun()

        pending = st.session_state.get("value_chain_pending_result") or {}
    ...
    if selected_chain_id is not None:
        result = load_value_chain_by_id_from_db(selected_chain_id)
    else:
        pending = st.session_state.get("value_chain_pending_result") or {}
        result = pending if pending.get("rows") else {}

    if not result or not result.get("rows"):
        st.info("이미지를 업로드해 분석하거나, 저장된 밸류체인을 선택하세요.")
        st.markdown("</div>", unsafe_allow_html=True)
        return
    ...
    if not match_rows:
        company_list_df_fix = load_company_list()
        match_rows = _build_value_chain_match_rows(result, company_list_df_fix)
    ...
    if selected_chain_id is not None:
        ...
        with sum_c2:
            ...
            if overview_source:
                st.caption(f"설명 검색소스: {overview_source}")
        if make_summary_btn:
            ai_provider, ai_api_key, ai_model = get_ai_settings_from_session("analysis")
            with st.spinner("구글 검색 기반으로 밸류체인 총 설명을 생성 중입니다..."):
                ...
                if summary_err:
                    ...
                    update_value_chain_in_db(selected_chain_id, chain_name, result)
                    st.session_state["value_chain_summary_notice"] = "밸류체인 총 설명 생성 완료."
            st.rerun()
    ...
    with view_tab:
        ...
        view_df = df[df["matched"] == True].copy() if bool(show_only_matched) else df.copy()  # noqa: E712

        company_list_df = load_company_list()
        analysis_all_df = load_company_analysis_history()
        latest_analysis_map: dict = {}
        if isinstance(analysis_all_df, pd.DataFrame) and not analysis_all_df.empty:
            ...
            for _, arow in tmp.iterrows():
                nm = str(arow.get("stock_name") or "").strip()
    ...
    with kw_tab:
        ...
        with kw_sel_col:
            if kw_type_filter == "단계":
                kw_pool = stage_kws
            elif kw_type_filter == "세부공정":
                kw_pool = segment_kws
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        if st.button("선택 날짜 신규 입력 시작(초기화)", key="sidebar_start_fresh_date_btn"):
            st.session_state["editing_df"] = ensure_portfolio_columns(
                empty_portfolio_df(), usd_krw_rate, force_usd_rate=True
            )
            st.session_state["editing_df_date"] = selected_date_key
            cash_key_suffix = selected_date.isoformat().replace("-", "")
            st.session_state[f"input_cash_krw_{cash_key_suffix}"] = 0.0
            st.session_state[f"input_cash_usd_{cash_key_suffix}"] = 0.0
            st.success(f"{selected_date} 신규 입력 모드로 초기화했습니다. (보유종목/예수금)")
